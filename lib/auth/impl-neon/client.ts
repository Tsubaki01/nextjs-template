...
export function useAuth(): AuthClient {
  // まず環境変数の有無を判定し、未設定ならば Stack のフックを呼ばずに早期リターンする
  const envReady = Boolean(
    process.env.NEXT_PUBLIC_STACK_PROJECT_ID && process.env.NEXT_PUBLIC_STACK_PUBLISHABLE_CLIENT_KEY
  );
  ...
  if (!envReady) {
    return {
      ...
      error: null,
      async signIn() {
        return { ok: false, error: 'STACK_ENV_MISSING' };
      },
      async signUp() {
        return { ok: false, error: 'STACK_ENV_MISSING' };
      },
  ...
  return {
    ...
    async verifyEmail(code: string) {
      try {
        ...
        return {
          ok: res.status === 'ok',
          error: res.status === 'ok' ? undefined : 'VERIFY_FAILED',
        };
      } catch (e: any) {
