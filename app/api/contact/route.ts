...
const rateLimitStore: Map<string, RateLimitEntry> = _global.__contactRateLimitStore || new Map();
...
export async function POST(request: NextRequest) {
  ...
  const { limited, remaining, resetMs } = checkAndConsumeRateLimit(`contact:${ip}`);
  if (limited) {
