...
export async function GET(req: NextRequest) {
  try {
    ...
    const isAlreadyUsed =
      statusCode === 409 || String(rawMsg).includes('VERIFICATION_CODE_ALREADY_USED');
    if (isAlreadyUsed) {
      return NextResponse.json({ ok: true, info: 'ALREADY_VERIFIED' });
    }
    const msg = rawMsg || 'VERIFY_FAILED';
    try {
      console.error('[Neon verify-email] failed:', res);
    } catch {}
    ...
    try {
      console.error('[Neon verify-email] exception:', e);
    } catch {}
