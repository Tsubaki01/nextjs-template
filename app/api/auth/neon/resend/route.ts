...
// Stack Auth SDKの型定義が不完全なため、このファイル全体でany型の使用を許可
...
export async function POST(req: NextRequest) {
  try {
    ...
    const derivedOrigin =
      (req.nextUrl && (req.nextUrl as any).origin) ||
    ...
    try {
      const primaryEmail =
        ...
        user.primaryEmail ||
        (typeof user.getPrimaryContactChannel === 'function'
          ? await user.getPrimaryContactChannel('email')
          : undefined);
    ...
    const channels: any[] =
      (typeof user.getContactChannels === 'function'
        ? await user.getContactChannels()
        : undefined) ||
      (typeof user.listContactChannels === 'function'
        ? await user.listContactChannels()
        : undefined) ||
      (Array.isArray(user.contactChannels) ? user.contactChannels : []);

    if (channels && channels.length) {
      const primary = channels.find(
        (c: any) => (c.type === 'email' || c.kind === 'email') && (c.isPrimary || c.primary)
      );
      const anyEmail =
        primary || channels.find((c: any) => c.type === 'email' || c.kind === 'email');
    ...
    try {
      console.error('[Neon resend-email] exception:', e);
    } catch {}
